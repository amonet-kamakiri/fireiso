...
make_basefs() {
    ...
    pacstrap -C "${script_path}/pacman.conf" -c -G -M "${work_dir}/x86_64/airootfs" $(pacman -Sqg base | sed 's/^linux$/linux-lts/' | uniq ; echo syslinux)
...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts -g /boot/fireiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_boot() {
    ...
    cp ${work_dir}/x86_64/airootfs/boot/fireiso.img ${work_dir}/iso/${install_dir}/boot/x86_64/fireiso.img
