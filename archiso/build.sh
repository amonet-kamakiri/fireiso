...
iso_label="FIREISO${iso_mainver//.}"
iso_publisher="FireISO"
iso_application="FireISO"
install_dir=fireiso
work_dir=work
out_dir=out
gpg_key=
...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts-kamakiri -g /boot/fireiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_boot() {
    ...
    cp ${work_dir}/${arch}/airootfs/boot/fireiso.img ${work_dir}/iso/${install_dir}/boot/${arch}/fireiso.img
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-lts-kamakiri ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
}
...
make_syslinux() {
    _uname_r=$(file -b ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-lts-kamakiri| awk 'f{print;f=0} /version/{f=1}' RS=' ')
